...
(define (mod-exp b e m)
 (cond ((= m 1) 0)
       ...
       ((and (> e 0) (= 1 (modulo e 2))) (modulo (* b (mod-exp b (- e 1) m)) m))))
...
(define (encrypthelp text key)
  (cond ((eq? text '()) '())
        (else (append (list(mod-exp (char->integer (car text)) (cadr key) (car key))) (encrypthelp (cdr text) key)))))
...
;; 6 PTS
(define (decrypthelp ciphertext key)
  (cond ((eq? ciphertext '()) '())
        (else (append (list (mod-exp (car ciphertext) (cadr key) (car key))) (decrypthelp (cdr ciphertext) key)))))

(define (decrypt ciphertext key)
  (list->string (map integer->char (decrypthelp ciphertext key))))
