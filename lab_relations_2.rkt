...
(define (compose S R)
  (cond ((null? S) '())
        ((null? R) '())
        ((equal? (cadr (car R)) (car (car S)))  (make-set (append (append (list (list (car (car R)) (cadr (car S)))) (compose (cdr S) R)) (compose S (cdr R)))))
        (else (make-set (append (compose (cdr S) R) (compose S (cdr R)))))))
...
;; 5 PTS
(define (powerhelp k R R0)
  (cond ((equal? k 0) '()) ((equal? k 1) R0)
        (else (powerhelp (- k 1) R (compose R R0)))))
...
;; 5 PTS
(define (transitive-closurehelp R k R0)
  (if (subset? (power k R) R0) R0 (transitive-closurehelp R (+ k 1) (union (power k R) R0))))

(define (transitive-closure R)
  (transitive-closurehelp R 1 '()))
