...
(define (relate x R)
  (if (null? R) '()
      (if (equal? x (car (car R))) (cons (car (cdr (car R))) (relate x (cdr R))) (append '() (relate x (cdr R))))))
...
(define (injective? R)
  (define (injective R)
    (cond ((null? R) '())
          (else (append(cdr(car R)) (injective (cdr R))))))
  (cond ((null? R) #t) ((subset? (list(car(injective R))) (cdr (injective R))) #f)
        (else (injective? (cdr R)))))
