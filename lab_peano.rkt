...
;; 1 PTS
(define (pred p)
  (cond ((pz? p) error-predecessor) (else (car p))))
...
;; 2 PTS
(define (p+ a b)
  (cond ((pz? a) b) ((pz? b) a) (else (p+ (succ a) (pred b)))))
...
;; 2 PTS
(define (p- a b)
  (cond ((pz? b) a) ((pz? a) error-subtraction) (else (p- (pred a) (pred b)))))
...
;; 2 PTS
(define (p* a b)
 (cond ((pz? b) z) ((pz? a) z) (else (p+ a (p* a (pred b))))))
...
;; 1 PTS
(define (p^ a b)
 (cond ((pz? b) (succ z)) ((pz? a) z) (else (p* a (p^ a (pred b))))))
...
;; 2 PTS
(define (p= a b)
 (cond ((and (pz? a) (pz? b)) #t) ((or (pz? a) (pz? b)) #f) (else (p= (pred a) (pred b)))))
...
;; 2 PTS
(define (p% a b)
 (cond ((p= a b) z) ((pz? b) error-modulo) ((pz? a) z) ((p> a b) (p% (p- a b) b))((p> b a) a)))
...
;; 3 PTS
(define (p-prime? p)
  (define (primehelper h p)
    (cond ((p= h (p- (succ p) p)) #t)
          ((p-divides? h p) #f)
          (else (primehelper (pred h) p))))
  (cond ((pz? p) #f) ((p= (succ z) p) #f) (else (primehelper (pred p) p))))