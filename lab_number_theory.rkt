...
;; 1 PTS
(define (lcm a b)
  (cond ((or (eq? a 0) (eq? b 0)) 0) (else (/ (* a b) (gcd a b)))))
...
(define (wj-solvable? cA cB g)
 (cond ((and (or (> cA g) (> cB g) (eq? g cA) (eq? g cB))
             (eq? 0 (modulo g (gcd cA cB)))) #t) (else #f)))
...
(define (waterjug cA cB g)
  (append (list (list 0 0)) (wj 0 0 cA cB g)))
(define (wj fA fB cA cB g)
  (cond ((eq? fB g) '())
        ((eq? fB cB) (cons (list fA 0) (wj fA 0 cA cB g)))
        ((eq? fA 0) (cons (list cA fB) (wj cA fB cA cB g)))
        ((> (+ fA fB) cB) (cons (list (- fA (- cB fB)) cB) (wj (- fA (- cB fB)) cB cA cB g)))
        (else (cons (list 0 (+ fB fA)) (wj 0 (+ fA fB) cA cB g)))))
...
;; 5 PTS
(define (factorize n)
 (define (factorizehelp n)
    (define (divisor n x)
      (cond ((eq? (modulo n x) 0) x)
            (else (divisor n (+ x 1)))))
    (cond ((eq? n 1) '())
          (else (append (list (divisor n 2)) (factorizehelp (/ n (divisor n 2)))))))
  (cond ((eq? n 1) (list 1))
        (else (factorizehelp n))))
...
;; 3 PTS
(define (cong-mod? m)
  (lambda (x y) (cond ((eq? (modulo x m) (modulo y m)) #t)
                      (else #f))))
