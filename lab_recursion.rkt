...
(define (subfact n)
  (cond ((= n 1) 0)
        ((= n 2) 1)
        ((and (< 2 n) (even? n)) (+(* n (subfact (- n 1))) 1))
        (else (-(* n (subfact (- n 1))) 1))))
...
(define (gen-fib a b n)
  (cond ((and (and (eq? a 'a) (eq? b 'b)) ((eq? n 'n))) 'G'(a,b,n))
        ((< n 0) '"Not nonnegative int")
        ((= n 0) a)
        ((= n 1) b)
        (else (+ (gen-fib a b (- n 1)) (gen-fib a b (- n 2))))))
